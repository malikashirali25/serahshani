...
// Theme preferences.
if ( ! \Vamtam_Elementor_Utils::is_widget_mod_active( 'vamtam-popup' ) ) {
	return;
}
...
class Vamtam_Popup extends DynamicTagsTag {
	...
	public function register_controls() {
		...
		$this->add_control(
			'use_vamtam_popup_toggle',
			[
				'label' => __( 'Use Vamtam Popup Toggle', 'vamtam-elementor-integration' ),
				'type' => Controls_Manager::SWITCHER,
				'condition' => [
					'action' => [ 'open', 'toggle' ],
				],
		...
		$this->add_control(
			'use_vamtam_popup_toggle_notice',
			[
				'type' => Controls_Manager::RAW_HTML,
				'raw' => __( '<i style="font-size:11px;line-height:1.4;color:#a4afb7;">Theme extension of the core Elementor popup toggle implementation. Use <strong>only</strong> with icon widget.</i>', 'vamtam-elementor-integration' ),
				'condition' => [
					'use_vamtam_popup_toggle!' => '',
				],
			]
	...
	private function print_open_popup_link( array $settings ) {
		...
		$link_action_url = Plugin::elementor()->frontend->create_action_hash( 'popup:open', [
			...
			'use_vamtam_popup_toggle' => $settings['use_vamtam_popup_toggle'],
		] );
